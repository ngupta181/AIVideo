...
const createVideo = async (mediaFiles, audioPath, outputPath, subtitles, manualOffset = 0, wordsPerSubtitle = 3) => {
    ...
    try {
        ...
        await createSubtitlesFile(subtitles, subtitlesPath, audioDuration);
        console.log('Subtitles file created');

        console.log('Synchronizing subtitles with audio...');
        await syncSubtitlesWithAudio(audioPath, subtitlesPath, wordsPerSubtitle);
        
        if (manualOffset !== 0) {
            console.log(`Applying manual offset of ${manualOffset} seconds`);
            await adjustSubtitles(subtitlesPath, manualOffset, wordsPerSubtitle);
        }
        ...
        console.log('Step 7: Adding audio...');
        ...
        const tempFiles = [concatenatedVideoPath, adjustedVideoPath, subtitledVideoPath];
...
const createSubtitlesFile = async (subtitles, outputPath, audioDuration) => {
    ...
    const lastSubtitleIndex = srtContent.lastIndexOf('\n\n');
    const lastSubtitle = srtContent.slice(lastSubtitleIndex).replace(/(\d+:\d+:\d+,\d+ --> )(\d+:\d+:\d+,\d+)/, `$1${formatSRTTime(audioDuration)}`);
    const finalSrtContent = srtContent.slice(0, lastSubtitleIndex) + lastSubtitle;

    await fs.writeFile(outputPath, finalSrtContent, { encoding: 'utf8' });
};
...
const applySubtitles = async (inputPath, subtitlesPath, outputPath) => {
    return new Promise((resolve, reject) => {
        ...
        // Escape the paths for ffmpeg
        const escapedSubtitlesPath = absoluteSubtitlesPath.replace(/\\/g, '/').replace(/:/g, '\\:');

        ffmpeg(absoluteInputPath)
            .outputOptions('-vf', `subtitles='${escapedSubtitlesPath}':force_style='FontSize=24'`)
